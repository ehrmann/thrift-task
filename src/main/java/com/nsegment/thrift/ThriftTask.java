...
public class ThriftTask extends MatchingTask {
    ...
    protected boolean allowNegKeys = false;
    protected boolean allow64bitConsts = false;
    ...
    public void execute() throws BuildException {
        ...
        if (allowNegKeys) {
            parameters.add("--allow-neg-keys");
        }
        if (allow64bitConsts) {
            parameters.add("--allow-64bit-consts");
        }

        // Include path
        if (this.includePath != null) {
            for (Resource resource : this.includePath) {
        ...
        // + 1 for a file placeholder
        String[] parameterArray = parameters.toArray(new String[parameters.size() + 1]);
        ...
        for (String file : filesToProcess) {
            ...
            // The other constructor is Thriftc(int pageSize, int totalPages, boolean exec)
            Thriftc thrift = new Thriftc();

            if (quiet) {
                int fd;
                fd = thrift.addFD(new Runtime.InputOutputStreamFD(NullOutputStream.NULL_OUTPUT_STREAM));
                if (thrift.sys_dup2(fd, 1) < 0) {
                    throw new BuildException(("Failed to redirect stdout"));
                }
                fd = thrift.addFD(new Runtime.InputOutputStreamFD(NullOutputStream.NULL_OUTPUT_STREAM));
                if (thrift.sys_dup2(fd, 2) < 0) {
                    throw new BuildException(("Failed to redirect stderr"));
                }
            }

            int status = thrift.run("thrift", parameterArray);

            if (status != 0) {
                throw new BuildException("thrift build failed.");
            }
        }
    }
    ...
    public void setAllowNegKeys(boolean val) {
        this.allowNegKeys = val;
    }

    public void setAllow64bitConsts(boolean val) {
        this.allow64bitConsts = val;
    }
